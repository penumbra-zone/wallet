...
export class ClientController {
            ...
            for (const statePayload of compactBlock.statePayloads) {
                try {
                    ...
                    if (statePayload.statePayload.case === "note") {
                        ...
                        decryptedNote.source = this.toHexString(statePayloadNote.source.inner);

                        if (decryptedNote.amount != 0) {
                            await this.indexedDb.putValue('notes', decryptedNote);
                        }
                        await this.saveTransaction(compactBlock.height, decryptedNote.source);
    ...
    requireScanning(compactBlock: CompactBlock) {
        return (
            compactBlock.statePayloads != null && compactBlock.statePayloads.length != 0
        );
    }
